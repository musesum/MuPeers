...
extension NWParameters {
    ...
    static func makeParamerters() -> NWParameters {
	...
	private static func tlsOptions(_ secret: String) -> NWProtocolTLS.Options {
		...
		let authenticationCode = HMAC<SHA256>.authenticationCode(for: "MuPeers".data(using: .utf8)!, using: authenticationKey)
...
//... the following is only in TLS 1.2, removing should allow TLS 1.3 if available
